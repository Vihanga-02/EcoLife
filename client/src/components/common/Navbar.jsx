        ...
        {isAuthenticated() ? (
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className="flex items-center gap-2 px-2 sm:px-3 py-2 rounded-lg hover:bg-green-50 transition-colors"
            >
              ...
              <span className="hidden sm:block text-black text-sm font-medium max-w-[160px] truncate">{user?.name}</span>
              <ChevronDown className={`text-gray-600 transition-transform ${dropdownOpen ? 'rotate-180' : ''}`} size={16} />
            </button>

            {dropdownOpen && (
              <div className="absolute right-0 mt-2 w-56 sm:w-48 bg-white border border-green-300 rounded-lg shadow-lg py-1 z-50">
